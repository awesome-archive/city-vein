...
gui.add(ops, 'trailWidth', 0.0, 2.0, 0.5).onChange(function (val) {
    options = myChart.getOption()
    options.series[0].lineStyle.normal.width = val;
    myChart.setOption(options);
});

gui.add(ops, 'trailLength', 0.0, 2.0, 0.5).onChange(function (val) {
    options = myChart.getOption();
    options.series[1].effect.trailLength = val;
    myChart.setOption(options);
});

gui.add(ops, 'pointSpeed', 0, 40, 10).onChange(function (val) {
    options = myChart.getOption();
    options.series[1].effect.constantSpeed = val;
    myChart.setOption(options);
});

gui.add(ops, 'pointSize', 0.0, 3.0, 1.0).onChange(function (val) {
    options = myChart.getOption();
